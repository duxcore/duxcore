...
export type ModifiedServiceTypeInstance = ServiceType & {
  services?: Service[];
  features: ServiceFeature[];
};

export default class ServiceTypeManager {
  private _raw: ServiceType & {
    services?: Service[];
    features: ServiceFeature[];
  };
  ...
  constructor(
    data: ServiceType & { services?: Service[]; features: ServiceFeature[] }
  ) {
    this._raw = data;
    ...
    if (this._raw.features)
      this._raw.features.map((v) =>
        this.features.set(v.id, new ServiceFeatureManager(v))
      );
  }

  toJson(includeServices = true) {
    return {
      id: this.id,
      name: this.name,
      services: includeServices ? this.services ?? undefined : undefined,
